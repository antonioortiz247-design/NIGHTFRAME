export function TransmissionFrame({ className = "", variant = "hero" }: { className?: string; variant?: "hero" | "portrait" | "product" }) {
  return (
    <div className={`transmission-frame group transmission-${variant} ${className}`}>
      ...
      <div className="absolute inset-0 bg-[linear-gradient(90deg,transparent_0_8%,rgba(255,255,255,0.05)_8.2%,transparent_8.4%_100%),linear-gradient(0deg,rgba(255,255,255,0.045)_1px,transparent_1px)] bg-[length:180px_100%,100%_9px] opacity-25" />
      <div className="absolute left-6 top-5 flex gap-2">
        <span className="h-2 w-2 rounded-full bg-signal-red shadow-[0_0_10px_rgba(193,18,31,0.55)]" />
